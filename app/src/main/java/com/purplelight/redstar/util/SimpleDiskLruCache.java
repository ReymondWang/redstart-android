...
public class SimpleDiskLruCache {
	 ...
	 * @return
	 */
	public static SimpleDiskLruCache openCache(File cacheDir, long maxByteSize) {
		if (!cacheDir.exists()) {
			if(!cacheDir.mkdirs()) {
		...
		if (cacheDir.isDirectory() && cacheDir.canWrite()
			...
			return new SimpleDiskLruCache(cacheDir, maxByteSize);
		}
	...
	/**
	 * Constructor that should not be called directly,creating a SimpleDiskLruCache instance.
	 *
	 * @param cacheDir
	 * @param maxByteSize
	 */
	private SimpleDiskLruCache(File cacheDir, long maxByteSize) {
		mCacheDir = cacheDir;
	...
	public File getFile(String key) {
		synchronized (mLinkedHashMap) {
			final String filePath = mLinkedHashMap.get(key);
			if (filePath != null) {
				if (DEBUG) {
					Log.d(TAG, "Disk cache hit");
				}
				...
				final String existingFilePath = createFilePath(mCacheDir, key);
				File file = new File(existingFilePath);
				if (file.exists()) {
					put(key, existingFilePath);
					if (DEBUG) {
						Log.d(TAG, "Disk cache hit (existing file)");
					}
					return file;
