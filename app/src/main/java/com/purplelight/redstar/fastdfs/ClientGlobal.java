...
public class ClientGlobal {
	 ...
	 */
	public static void init() throws IOException, McmException {
		String[] szTrackerServers;
		String[] parts;

		g_connect_timeout = Configuration.Fastdfs.CONNECT_TIMEOUT;
		if (g_connect_timeout < 0) {
			g_connect_timeout = DEFAULT_CONNECT_TIMEOUT;
		}
		g_connect_timeout *= 1000; // millisecond

		g_network_timeout = Configuration.Fastdfs.NETWORK_TIMEOUT;
		if (g_network_timeout < 0) {
			g_network_timeout = DEFAULT_NETWORK_TIMEOUT;
		}
		g_network_timeout *= 1000; // millisecond

		g_charset = Configuration.Fastdfs.CHARSET;
		if (g_charset == null || g_charset.length() == 0) {
			g_charset = "ISO8859-1";
		}

		szTrackerServers = Configuration.Fastdfs.TRACKER_SERVER.split(";");
		if (szTrackerServers == null) {
			throw new McmException("item \"tracker_server\" is not found");
		}
		...
		g_tracker_http_port = Configuration.Fastdfs.TRACKER_HTTP_PORT;
		g_anti_steal_token = Configuration.Fastdfs.ANTI_STEAL_TOKEN;
		if (g_anti_steal_token) {
			g_secret_key = Configuration.Fastdfs.SECRET_KEY;
		}
	}
